<p align="center">
  <img src="main/assets/voltaria-banner.png" width="100%">
</p>

<h1 align="center">⚡ VOLTARIA NEXUS ⚡</h1>

<p align="center">
  <i>A Powerful WhatsApp Bot with Economy, Games & Moderation — built with Baileys Multi-Device</i>
</p>

<p align="center">
  <a href="https://github.com/akiraxyd-005/Voltaria-v2.0/fork">
    <img src="https://img.shields.io/github/forks/akiraxyd-005/Voltaria-v2.0?label=Fork&style=social">
  </a>
  &nbsp;
  <a href="https://github.com/akiraxyd-005/Voltaria-v2.0/stargazers">
    <img src="https://img.shields.io/github/stars/akiraxyd-005/Voltaria-v2.0?style=social">
  </a>
</p>

<p align="center">
  <a href="https://github.com/Arashi005">
    <img src="https://img.shields.io/badge/Owner-Arashi-white.svg?style=for-the-badge&logo=github">
  </a>
  <a href="https://github.com/akiraxyd-005/Voltaria-v2.0/blob/main/LICENSE">
    <img src="https://img.shields.io/github/license/akiraxyd-005/Voltaria-v2.0?color=%231e81b0&style=for-the-badge">
  </a>
</p>

<p align="center">
  <a href="https://whatsapp.com/channel/0029VbD6zwN4tRrvLtUTya0I">
    <img src="https://img.shields.io/badge/Join%20Channel-25D366?style=for-the-badge&logo=whatsapp&logoColor=white">
  </a>
  <a href="https://chat.whatsapp.com/E5vC8SxmkKXJ0ufWRwIc3l">
    <img src="https://img.shields.io/badge/Join%20Community-25D366?style=for-the-badge&logo=whatsapp&logoColor=white">
  </a>
</p>

---

## ⚠️ Disclaimer

> **Important:** This bot is for educational purposes only. Using automated bots on WhatsApp violates WhatsApp's Terms of Service. Your WhatsApp account may get banned. The developer is **not responsible** for any account bans, restrictions, or legal issues that may arise from using this bot. Use at your own risk.

---

## 🎀 Features

| Category | Details |
|----------|---------|
| 💰 **Economy** | Currency, bank, cards, gambling, shop, leaderboard |
| 🎮 **Games** | Word Chain, Trivia, Tic Tac Toe, Hangman, Wordle |
| 👥 **Group** | Welcome/Goodbye, antilink, warn/kick, tagall, polls |
| 🤖 **AI** | Gemini AI, GPT chat, image generation |
| 📥 **Download** | YouTube, TikTok, Instagram, Twitter, Spotify |
| 🎭 **Reactions** | Hug, kiss, pat, slap, dance (GIFs) |
| 🎵 **Audio** | Voice effects (deep, robot, nightcore, etc.) |
| 👑 **Owner** | Broadcast, restart, ban, sudo, mode |

---

## 🚀 Deployment

### Deploy on Railway

[![Deploy on Railway](https://railway.app/button.svg)](https://railway.app/template/your-template-link)

### Local Installation

```bash
git clone https://github.com/akiraxyd-005/Voltaria-v2.0
cd Voltaria-v2.0
npm install
npm start
```

Scan the QR code via WhatsApp → Settings → Linked Devices → Link a Device.

---

📝 Type §menu in WhatsApp to see all commands

---

👤 Author

Arashi - GitHub

---

📜 License

MIT License

---

<p align="center">
  <b>©𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝙽£𝚇𝚄$</b>
</p>
```
