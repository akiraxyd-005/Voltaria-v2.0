...
module.exports = {
    name: 'antistatus',
    category: 'admin',
    description: 'Set anti-status mention mode (off/delete/warn/kick)',
    usage: '§antistatus off | delete | warn | kick',
    isGroup: true,
    isAdmin: true,
    async execute(sock, msg, args, extra) {
        ...
        if (!mode || !validModes.includes(mode)) {
            return extra.reply(`❓ *𝑈𝑠𝑎𝑔𝑒:* §𝑎𝑛𝑡𝑖𝑠𝑡𝑎𝑡𝑢𝑠 𝑜𝑓𝑓 | 𝑑𝑒𝑙𝑒𝑡𝑒 | 𝑤𝑎𝑟𝑛 | 𝑘𝑖𝑐𝑘

• *𝑑𝑒𝑙𝑒𝑡𝑒* - 𝐷𝑒𝑙𝑒𝑡𝑒 @𝑠𝑡𝑎𝑡𝑢𝑠 𝑚𝑒𝑛𝑡𝑖𝑜𝑛𝑠
...
• *𝑘𝑖𝑐𝑘* - 𝐼𝑛𝑠𝑡𝑎𝑛𝑡𝑙𝑦 𝑘𝑖𝑐𝑘 𝑢𝑠𝑒𝑟`);
        ...
        settings[extra.from].antistatus = mode;
        fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));
        
        const modeEmoji = mode === 'off' ? '❌' : mode === 'delete' ? '🗑️' : mode === 'warn' ? '⚠️' : '👢';
        await extra.reply(`${modeEmoji} *𝐴𝑛𝑡𝑖-𝑆𝑡𝑎𝑡𝑢𝑠 𝑚𝑜𝑑𝑒 𝑠𝑒𝑡 𝑡𝑜: ${mode.toUpperCase()}*`);
    }
};