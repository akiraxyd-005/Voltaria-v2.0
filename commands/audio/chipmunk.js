...
module.exports = {
    name: 'chipmunk',
    category: 'audio',
    description: 'Apply chipmunk effect to audio/video',
    usage: '§chipmunk (reply to an audio message)',
    async execute(sock, msg, args, extra) {
        const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
        
        if (!quoted || (!quoted.audioMessage && !quoted.voiceMessage && !quoted.videoMessage)) {
            return extra.reply('❌ Please reply to an audio or voice message.\n\nUsage: Reply to an audio with §chipmunk');
        }
        
        await extra.reply('🐿️ Processing audio with *chipmunk* effect...');
        
        try {
            const audioBuffer = await downloadMedia(sock, quoted);
            const processedBuffer = await applyAudioEffect(audioBuffer, 'chipmunk');
            
            await sock.sendMessage(extra.from, {
                audio: processedBuffer,
                mimetype: 'audio/mpeg',
                fileName: 'chipmunk_effect.mp3',
                ptt: true
            }, { quoted: msg });
        } catch (error) {
