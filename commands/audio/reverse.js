...
module.exports = {
    name: 'reverse',
    category: 'audio',
    description: 'Apply reverse effect to audio/video',
    usage: '§reverse (reply to an audio message)',
    async execute(sock, msg, args, extra) {
        const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
        
        if (!quoted || (!quoted.audioMessage && !quoted.voiceMessage && !quoted.videoMessage)) {
            return extra.reply('❌ Please reply to an audio or voice message.\n\nUsage: Reply to an audio with §reverse');
        }
        
        await extra.reply('🔄 Processing audio with *reverse* effect...');
        
        try {
            const audioBuffer = await downloadMedia(sock, quoted);
            const processedBuffer = await applyAudioEffect(audioBuffer, 'reverse');
            
            await sock.sendMessage(extra.from, {
                audio: processedBuffer,
                mimetype: 'audio/mpeg',
                fileName: 'reverse_effect.mp3',
                ptt: true
            }, { quoted: msg });
        } catch (error) {
