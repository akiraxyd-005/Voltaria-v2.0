...
module.exports = {
    name: 'blown',
    category: 'audio',
    description: 'Apply blown effect to audio/video',
    usage: '§blown (reply to an audio message)',
    async execute(sock, msg, args, extra) {
        const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
        
        if (!quoted || (!quoted.audioMessage && !quoted.voiceMessage && !quoted.videoMessage)) {
            return extra.reply('❌ Please reply to an audio or voice message.\n\nUsage: Reply to an audio with §blown');
        }
        
        await extra.reply('🎵 Processing audio with *blown* effect...');
        
        try {
            const audioBuffer = await downloadMedia(sock, quoted);
            const processedBuffer = await applyAudioEffect(audioBuffer, 'blown');
            
            await sock.sendMessage(extra.from, {
                audio: processedBuffer,
                mimetype: 'audio/mpeg',
                fileName: 'blown_effect.mp3',
                ptt: true
            }, { quoted: msg });
        } catch (error) {
