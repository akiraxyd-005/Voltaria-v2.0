...
module.exports = {
    name: 'fast',
    category: 'audio',
    description: 'Apply fast effect to audio/video',
    usage: '§fast (reply to an audio message)',
    async execute(sock, msg, args, extra) {
        const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
        
        if (!quoted || (!quoted.audioMessage && !quoted.voiceMessage && !quoted.videoMessage)) {
            return extra.reply('❌ Please reply to an audio or voice message.\n\nUsage: Reply to an audio with §fast');
        }
        
        await extra.reply('⚡ Processing audio with *fast* effect...');
        
        try {
            const audioBuffer = await downloadMedia(sock, quoted);
            const processedBuffer = await applyAudioEffect(audioBuffer, 'fast');
            
            await sock.sendMessage(extra.from, {
                audio: processedBuffer,
                mimetype: 'audio/mpeg',
                fileName: 'fast_effect.mp3',
                ptt: true
            }, { quoted: msg });
        } catch (error) {
