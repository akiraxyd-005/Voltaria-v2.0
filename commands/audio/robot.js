...
module.exports = {
    name: 'robot',
    category: 'audio',
    description: 'Apply robot effect to audio/video',
    usage: '§robot (reply to an audio message)',
    async execute(sock, msg, args, extra) {
        const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
        
        if (!quoted || (!quoted.audioMessage && !quoted.voiceMessage && !quoted.videoMessage)) {
            return extra.reply('❌ Please reply to an audio or voice message.\n\nUsage: Reply to an audio with §robot');
        }
        
        await extra.reply('🤖 Processing audio with *robot* effect...');
        
        try {
            const audioBuffer = await downloadMedia(sock, quoted);
            const processedBuffer = await applyAudioEffect(audioBuffer, 'robot');
            
            await sock.sendMessage(extra.from, {
                audio: processedBuffer,
                mimetype: 'audio/mpeg',
                fileName: 'robot_effect.mp3',
                ptt: true
            }, { quoted: msg });
        } catch (error) {
