...
module.exports = {
    name: 'highfive', category: 'reactions', description: 'High five someone', usage: '§highfive @user',
    async execute(sock, msg, args, extra) {
        const t = msg.mentionedJid?.[0]||null;
        if(!t) return extra.reply('❌ Mention someone!\nUsage: §highfive @user');
        try {
            const url = await gif('handhold');
            await sock.sendMessage(msg.chat,{video:{url},gifPlayback:true,caption:`@${extra.sender.split('@')[0]} high-fives @${t.split('@')[0]} ✋${FOOTER}`,mentions:[extra.sender,t]},{quoted:msg});
