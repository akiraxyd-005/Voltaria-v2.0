...
module.exports = {
    name: 'hug', category: 'reactions', description: 'Send a hug GIF', usage: '§hug @user',
    async execute(sock, msg, args, extra) {
        const t = msg.mentionedJid?.[0]||null;
        try {
            const url = await gif('hug');
            const cap = t ? `@${extra.sender.split('@')[0]} hugs @${t.split('@')[0]} 🤗${FOOTER}` : `@${extra.sender.split('@')[0]} sends a hug 🤗${FOOTER}`;
