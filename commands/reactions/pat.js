...
module.exports = {
    name: 'pat', category: 'reactions', description: 'Pat someone on the head', usage: '§pat @user',
    async execute(sock, msg, args, extra) {
        const t = msg.mentionedJid?.[0]||null;
        try {
            const url = await gif('pat');
            const cap = t ? `@${extra.sender.split('@')[0]} pats @${t.split('@')[0]} 🥺${FOOTER}` : `@${extra.sender.split('@')[0]} pats the air 🥺${FOOTER}`;
